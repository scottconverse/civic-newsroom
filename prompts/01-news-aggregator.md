...
- NOTE: Use journalism as LEADS ONLY — not as sources. See Prompt 08
  (Civic Grounding Protocol) for full anti-plagiarism safeguards.
...
SOURCE TIER CLASSIFICATION (MANDATORY):
For EACH source URL, classify tier and document:
- [URL] → TIER: [A/B/C] → TYPE: [.gov record / journalism / social media / other]
  ...
  not yet posted, check the city's YouTube channel for the recording and use
  the auto-generated transcript to ground claims. Only Tier A sources ground
  published claims. Full definitions: templates/source-tier-reference.md)
- [URL] → TIER: [A/B/C] → TYPE: [...]

PRIMARY SOURCE GROUNDING STATUS:
☐ At least one Tier A source independently confirms the core factual claim
☐ No Tier A source available — this lead REQUIRES primary source verification
  before story expansion (see Prompt 08: Civic Grounding Protocol)

If no Tier A source is available:
- Flag lead as UNGROUNDED
- Note the specific Tier A document that WOULD ground it (agenda item number,
  meeting minutes date, permit filing, budget line)
- Note the mechanism to obtain it (PrimeGov portal, official YouTube transcript,
  city clerk, CORA request)
- Lead may proceed to Story Expansion ONLY after Tier A source is obtained
  OR lead must be converted to HEADLINE-ONLY NOTE at expansion stage

PRIMARY SOURCE:
[Direct URL to council packet, news announcement, or official document]
[TIER A SOURCES ONLY — journalism URLs go in Secondary Sources]

SECONDARY SOURCES:
[Cross-reference URLs — journalism, Tier B/C sources as LEADS ONLY]
...
- Tier A (Daily check): Official government portals, council agendas, court records
- Tier B (Weekly check): School district, transit, county news pages, planning portals
- Tier C (Conditional): Community forums, social media, local media, business registries
...
RECOMMENDATION:
ADD TO TIER A (Daily check) - Confidence >= 0.85
ADD TO TIER B (Weekly check) - Confidence 0.75-0.84
ADD TO TIER C (Conditional check) - Confidence 0.65-0.74
...
Tier A Sources to Check: [Specific URLs or portal queries]
```
